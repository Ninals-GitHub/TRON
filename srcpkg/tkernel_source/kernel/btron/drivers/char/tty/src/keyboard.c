...
#define	KBD_IN_EX_EVENT		0x00000002
...
repeat:
	...
	/* wait for a data							*/
	err = tk_wai_flg(kbd_state.flgid, KBD_IN_EVENT | KBD_IN_EX_EVENT,
			TWF_ORW | TWF_CLR, &flgptn, TMO_FEVR);
	
	if (UNLIKELY(flgptn & KBD_IN_EX_EVENT)) {
		return(0);
	}
...
LOCAL void ex0_2nd_code(int code)
{
	...
	case	SCODE_MK_2_L_ARROW:
		tty_notify_arrow(TTY_ARROW_LEFT);
		
		err = _tk_set_flg(kbd_state.flgid, KBD_IN_EX_EVENT);
	...
	case	SCODE_MK_2_DN_ARROW:
		tty_notify_arrow(TTY_ARROW_DOWN);
	...
	case	SCODE_MK_2_R_ARROW:
		tty_notify_arrow(TTY_ARROW_RIGHT);
