...
EXPORT ER elf_load( ProgInfo *pg, LoadSource *ldr, UINT attr, Elf32_Ehdr *hdr )
{
	...
	/* map text area							*/
	/* -------------------------------------------------------------------- */
	map_size = (unsigned long)eli.text_ladr - PAGE_ALIGN(eli.text_ladr);
	map_size += (unsigned long)lofs - PAGE_ALIGN(lofs);
	map_size += eli.text_size;
#if 0
	printf("text:map_size 0x%08X\n", map_size);
#endif
	...
	/* Read text area */
#if 0
	printf("ldr:0x%08X ", ldr);
	printf("eli.text_fofs:0x%08X ",  eli.text_fofs);
	printf("eli.text_ladr + lofs:0x%08X ", eli.text_ladr + lofs);
	printf("eli.text_size:0x%08X\n", eli.text_size);
#endif
	...
	/* map data area							*/
	/* -------------------------------------------------------------------- */
	map_size = (unsigned long)eli.data_ladr - PAGE_ALIGN(eli.data_ladr);
	map_size += (unsigned long)lofs - PAGE_ALIGN(lofs);
	...
	/* 0-clear bss area							*/
	/* -------------------------------------------------------------------- */
	memset((void*)eli.bss_ladr, 0x00, eli.bss_size);
#if 0
	printf("eli.bss_ladr:0x%08X ", eli.bss_ladr);
	printf("eli.bss_size:0x%08X\n", eli.bss_size);
	
	printf("entry : 0x%08X\n", (B*)hdr->e_entry + lofs);
	printf("mspace->start_code : 0x%08X\n", mspace->start_code + lofs);
#endif
...
==================================================================================
 ...
 Input		:struct process *proc
 		 < a process of which stack to be set up>
			...
			struct LoadSource* ldr, ELF_LoadInfo *eli, int argc)
{
	...
	/* end marker								*/
	/* -------------------------------------------------------------------- */
	stack_top = stack_up(stack_top, 0);
	...
	if (stack_top & 0xF) {
		...
		stack_top = (unsigned long)top;
	}
	//printf("padding:0x%08X ", stack_top);
	/* -------------------------------------------------------------------- */
	/* set up auxv vector							*/
	/* -------------------------------------------------------------------- */
	...
	kfree(argvp);
	mspace->start_arg = stack_top;
	//printf("argv vec:start = 0x%08X ", mspace->start_arg);
	//printf("end = 0x%08X\n", mspace->end_arg);
	/* -------------------------------------------------------------------- */
	/* set up argc								*/
	/* -------------------------------------------------------------------- */
