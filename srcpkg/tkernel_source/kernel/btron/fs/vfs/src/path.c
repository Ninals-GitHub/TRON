...
EXPORT struct qstr* dup_pathname(const char *pathname)
{
	...
	new_path->len = strncpy_from_user(new_path->name, pathname, PATH_MAX);
	
	if (UNLIKELY(!new_path->len)) {
		vd_printf("dup_pathname:cannot copy\n");
		goto error_out;
	}
...
next_parse:
	...
	vd_printf("parse_path:%s\n", entry_name.name);
	if (dir->d_name.name) {
		vd_printf("dir name:%s\n", dir->d_name.name);
	} else {
		vd_printf("dir name:%s", dir->d_iname);
	}
