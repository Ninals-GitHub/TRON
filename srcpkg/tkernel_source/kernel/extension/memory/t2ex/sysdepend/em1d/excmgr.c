...
IMPORT	void	*lowmem_top;
	ER	ercd;

	/* Maximum number of simultaneous page faults allowed */
	ercd = _tk_get_cfn((UB*)"MaxPageFault", (INT*)&MaxMemFault, 1);
	if ( ercd <= 0 || MaxMemFault <= 0 ) 
		MaxMemFault = 64;

	/* Allocate exception message buffer */
	MemFault_buf = (MemFaultInfo*)(((UW)lowmem_top + 0x3) & ~0x3);
	lowmem_top = MemFault_buf + MaxMemFault;
#endif
