...
/*
 *  fs_unlink() - Unlink file
 */
EXPORT	INT	xfs_unlink(fs_env_t *env, const B *name)
{
	fs_path_t	path;
	INT		sts;
	...
	/* Call FIMP UNLINK service */
	env->t_cmd.r_unlink.path = path.p_name;
	sts = fs_callfimp(env, path.p_con, FIMP_UNLINK);
exit1:
